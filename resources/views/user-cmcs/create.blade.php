<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="font-bold text-2xl text-slate-800 dark:text-slate-100 leading-tight">{{ __('Nouveau profil CMC') }}</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Créez un nouveau profil de conseiller pour votre compte utilisateur.</p>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/50 p-6 sm:p-8">
                ...
                <form method="POST" action="{{ route('user-cmcs.store') }}" class="space-y-6">
                    @csrf

                    <div class="space-y-4">
                        <div>
                            <x-input-label for="post" :value="__('Poste CMC')" />
                            <x-text-input id="post" name="post" type="text" class="mt-1 block w-full" :value="old('post')" required autofocus placeholder="Ex: Conseiller d'orientation, Directeur..." />
                            <x-input-error :messages="$errors->get('post')" class="mt-2" />
                            <p class="mt-2 text-xs text-slate-400 dark:text-slate-500">
                                {{ __('Saisissez l\'intitulé de votre poste professionnel au sein du Centre de Métiers CMC.') }}
                            </p>
                        </div>
                    </div>
