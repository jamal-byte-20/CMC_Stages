<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="font-bold text-2xl text-slate-800 dark:text-slate-100 leading-tight">{{ __('Créer un partenaire') }}</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Ajoutez un nouveau partenaire entreprise à votre espace CMC.</p>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/50 p-6 sm:p-8">
                ...
                <form method="POST" action="{{ route('partenaires.store') }}" class="space-y-6">
                    @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" required autofocus placeholder="Ex: Opcio SA" />
                        ...
                        <div>
                            ...
                            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email')" required placeholder="Ex: contact@opcio.com" />
                        ...
                        <div class="grid sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="phone" name="phone" type="text" class="mt-1 block w-full" :value="old('phone')" placeholder="Ex: +212 522 000000" />
                                <x-input-error :messages="$errors->get('phone')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="city" :value="__('Ville')" />
                                <x-text-input id="city" name="city" type="text" class="mt-1 block w-full" :value="old('city')" placeholder="Ex: Casablanca" />
                        ...
                        <div>
                            ...
                            <x-text-input id="address" name="address" type="text" class="mt-1 block w-full" :value="old('address')" placeholder="Ex: 120 Bd d'Anfa, 3ème étage" />
